...
If dev and prod share the same CIDR (both 10.0.0.0/16), you can never peer them or connect them to a shared services VPC — the routes would be ambiguous. Non-overlapping ranges also let you write security group rules that explicitly allow or deny cross-environment traffic, and a source IP alone tells you which environment it came from.
